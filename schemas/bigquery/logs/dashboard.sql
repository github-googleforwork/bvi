-- dashboard (view)
SELECT
  DATE(pre_dash.side1.date) AS report_date,
  DATE(pre_dash_time) AS run_date,
  IF(pre_dash.side1.resource == 'exec_manager', "==============", pre_dash.side1.resource) AS resource,
  pre_dash.side1.start_time AS start_time,
  pre_dash.side1.end_time AS end_time,
  INTEGER(ROUND((pre_dash.side1.end_time - pre_dash.side1.start_time)/1000000, 0)) AS elapsed,
  IF(pre_dash.side1.resource == 'exec_manager', "========", IF(pre_dash.side1.end_time IS NULL, "RUNNING", IF(first_error <= pre_dash_time
        AND count_errors > 0
  ...
  IF(pre_dash.side1.resource == 'exec_manager', "===============", error_id) AS error_id
FROM (
  SELECT
  ...
  FROM (
    ...
    JOIN (
      ...
      FROM
        [YOUR_PROJECT_ID:logs.pre_dashboard]
      WHERE
        side1.resource = 'exec_manager' ) max_execs
    ON
