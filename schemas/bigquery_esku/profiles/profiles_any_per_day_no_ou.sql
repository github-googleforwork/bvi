...
SELECT
  ...
  MAX(CASE
      WHEN (event_name IN ('edit') AND (owner IS NOT NULL) AND (email <> owner)) THEN 1
      ELSE 0 END) AS is_collaborator,
  MAX(CASE
      WHEN event_name IN ('view') THEN 1
      ELSE 0 END) AS is_consumer,
  MAX(CASE
      WHEN (event_name IN ('change_document_visibility') AND (visibility_change = 'external')) THEN 1
