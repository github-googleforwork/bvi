...
SELECT
  ...
  user_usage.num_sheets_created + user_usage.num_sheets_edited + user_usage.num_sheets_trashed + user_usage.num_sheets_viewed AS num_spreadsheets,
...
FROM (
  SELECT
    ...
    drive.num_google_spreadsheets_created  AS num_sheets_created,
    drive.num_google_spreadsheets_edited AS num_sheets_edited,
    drive.num_google_spreadsheets_trashed AS num_sheets_trashed,
    drive.num_google_spreadsheets_viewed AS num_sheets_viewed
  FROM
    [YOUR_PROJECT_ID:EXPORT_DATASET.usage]
  WHERE
    TRUE
    AND _PARTITIONTIME = YOUR_TIMESTAMP_PARAMETER
    AND (drive.num_google_spreadsheets_created + drive.num_google_spreadsheets_edited + drive.num_google_spreadsheets_trashed + drive.num_google_spreadsheets_viewed) > 0
