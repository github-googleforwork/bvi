...
SELECT
  ...
  user_usage.num_preso_created + user_usage.num_preso_edited + user_usage.num_preso_trashed + user_usage.num_preso_viewed AS num_presentations,
...
FROM (
  SELECT
    ...
    drive.num_google_presentations_created  AS num_preso_created,
    drive.num_google_presentations_edited AS num_preso_edited,
    drive.num_google_presentations_trashed AS num_preso_trashed,
    drive.num_google_presentations_viewed AS num_preso_viewed
  FROM
    [YOUR_PROJECT_ID:EXPORT_DATASET.usage]
  WHERE
    TRUE
    AND _PARTITIONTIME = YOUR_TIMESTAMP_PARAMETER
    AND (drive.num_google_presentations_created + drive.num_google_presentations_edited + drive.num_google_presentations_trashed + drive.num_google_presentations_viewed) > 0
