...
SELECT
  ...
  user_usage.num_docs_created + user_usage.num_docs_edited + user_usage.num_docs_trashed + user_usage.num_docs_viewed AS num_text_documents,
...
FROM (
  SELECT
    ...
    drive.num_google_documents_created  AS num_docs_created,
    drive.num_google_documents_edited AS num_docs_edited,
    drive.num_google_documents_trashed AS num_docs_trashed,
    drive.num_google_documents_viewed AS num_docs_viewed
  FROM
    [YOUR_PROJECT_ID:EXPORT_DATASET.usage]
  WHERE
    TRUE
    AND _PARTITIONTIME = YOUR_TIMESTAMP_PARAMETER
    AND (drive.num_google_documents_created + drive.num_google_documents_edited + drive.num_google_documents_trashed + drive.num_google_documents_viewed) > 0
